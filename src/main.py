...
def parcels_simulations(wind, rise, alpha, theta=1.0, gamma=1.0):
    ...
    :return:
    """
    utils.simulation_description(
        wind, rise, alpha, theta, gamma, diffusion, wave_roughness, boundary
    )
    ...
    concentration_file = utils.get_concentration_output_name(
        wind,
        rise,
        diffusion,
        boundary,
    ...
    if not utils.check_file_exist(concentration_file):
        parcels_simulation_functions.vertical_diffusion_run(
            wind,
            rise,
...
def basic_statistics(wind, rise, alpha, theta=1.0, gamma=1.0):
    ...
    analysis.correlation_field_model_data(
        wind,
        rise,
        diffusion,
        boundary,
    ...
    analysis.mean_percentage_deviation(
        wind=wind,
        rise=rise,
        diffusion=diffusion,
        boundary=boundary,
    ...
    analysis.rise_velocity_turbulence_ratio(
        wind,
        rise,
        diffusion,
        theta=theta,
...
def plotting():
    ...
    visualization.just_diffusion_profile(
    ...
    visualization.gamma_sensitivity(
        w_rise=-0.0003, close_up=(0, -20), with_observations=False
    ).plot()
        ...
        for rise in w_rise:
            for alpha_val in alpha:
                if diffusion == "KPP":
                    ...
                    for g in gamma:
                        parcels_simulations(
                            wind=wind, rise=rise, alpha=alpha_val, gamma=g
                        )
                        basic_statistics(
                            wind=wind, rise=rise, alpha=alpha_val, gamma=g
                        )
